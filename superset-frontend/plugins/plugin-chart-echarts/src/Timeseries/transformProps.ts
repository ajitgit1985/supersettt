...
import { quarterNames, quarterSorting } from 'plugins/helpingMethoads/methods';
...
export default function transformProps(
  ...
  const quarterValue = quarterNames.includes(formData.xAxis);
  if (quarterValue) {
    const dataValue = echartOptions?.series[0]?.data;
    quarterSorting(dataValue);
  }
  return {
    showChartFilter,
    echartOptions,
