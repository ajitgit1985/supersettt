...
const SupersetPluginChartHelloWorld: React.FC<SupersetPluginChartHierarchyProps> =
  (props: any) => {
    ...
    useEffect(() => {
      ...
      cleanedDataArray.forEach((entry: any) => {
    ...
    const renderAllStates = () => {
      return sortedStates.map((state: string) => {
        ...
        return (
          <>
            <text style={{ fontSize: 12, fontWeight: 'bold', marginLeft: 10 }}>
              {state}
            </text>
            <Bar
              width={width}
    ...
    const dynamicDatasetsForBar1 = dynamicKeys.map((key, index) => ({
      ...
      borderWidth: 1,
      backgroundColor: convertedData?.map(item => {
        let b;
    ...
    const [scrollEnable, setScrollEnable] = useState('none');
