...
import { regexFormats } from '../../helpingMethoads/methods';
...
function extractDetails(q, regex) {
  const match = q.match(regex);
  const quarter = parseInt(match[1], 10);
  const yearStart = parseInt(match[2], 10);
  const yearEnd = yearStart + parseInt(match[3], 10);
  return { quarter, yearStart, yearEnd };
}
function cmp(a, b) {
  let regex;
  if (a.match(regexFormats.quarterYear)) {
  ...
  if (matchA && matchB) {
    const {
      quarter: quarterA,
      yearStart: yearStartA,
      yearEnd: yearEndA,
    } = extractDetails(a, regex);
    const {
      quarter: quarterB,
      yearStart: yearStartB,
      yearEnd: yearEndB,
    } = extractDetails(b, regex);

    if (yearStartA !== yearStartB) {
      return yearStartA - yearStartB;
    }

    if (yearEndA !== yearEndB) {
      return yearEndA - yearEndB;
    }
...
function Heatmap(element, props) {
  ...
  if (showValues) {
    ...
    cells
      ...
      .attr(
        'font-size',
        `${Math.min(yRbScale.rangeBand(), xRbScale.rangeBand()) / 5}px`,
      )
