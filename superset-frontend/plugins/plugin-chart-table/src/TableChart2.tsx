 ...
 */
// console.log('color', colorPositiveNegative);
...
function cellBackground({
  ...
  const r = colorPositiveNegative && value < 0 ? 150 : 225;
...
//   let convert_color;
//   for (let i = 0; i < 3; i++) {
//     console.log('colorsvalues', hex[i]);
//     const hexColor = hex[i].replace(/^#/, ''); // Remove the '#' if present
//     const bigint = parseInt(hexColor, 16);
//     const r = (bigint >> 16) & 255;
//     const g = (bigint >> 8) & 255;
//     const b = bigint & 255;

//     convert_color = `rgba(${r},${g},${b},${alpha})`;
//   }
...
//   colorPositiveNegative = false,
//   fixedRandomColors,
//   hexColorCode = '#58D601', // Default hex color code (you can change it)
// }: {
//   value: number;
//   colorPositiveNegative: boolean;
//   hexColorCode?: string;
//   fixedRandomColors: any;
// }) {
//   const alpha = 0.5; // Alpha value for RGBA

//   if (colorPositiveNegative && value < 0) {
//     return hexToRGBA(hexColorCode, alpha);
//   } else {
//     // Default color if not positive and negative
//     return hexToRGBA(fixedRandomColors, alpha); // You can change the default color if needed
//   }
// }
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  const linearColorScale = CategoricalColorNamespace.getScale(color_scheme);
  const fixedRandomColors = linearColorScale.colors;
  // console.log('linearColorScale', fixedRandomColors);
    ...
    (column: DataColumnMeta, i: number): ColumnWithLooseAccessor<D> => {
      ...
      return {
        ...
        Cell: ({ value, row }: { value: DataRecordValue; row: Row<D> }) => {
          ...
          // const a = i - 1;
          // let b;
          // const a1 = parseFloat('0.' + row.index);
          // if (row.index <= 6) {
          //   b = 0.9 - a1;
          // } else {
          //   b = 0.2;
          ...
          const colorMap = ['green', 'red', 'gray', 'pink'];
          // const colorMap = `${fixedRandomColors.length} `;

          // console.log('value', colorMap);

          const colorMap1 = colorMap.length;
          const dividedByThree = Math.floor(count1 / colorMap1);
          // const count2 = data;
          if (row.index <= 5) {
            // Calculate opacity based on the total index divided by 3
            b = 0.8 - a1 * 0.7; // Linearly change opacity from 0.9 to 0.2
          } else {
            b = 0.2; // Opacity becomes 0.2 for higher indices
          }

          // Calculate color index based on total index divided by 3
          // const colorMap = ['green', 'red', 'gray'];

          const colorIndex = Math.floor(row.index / dividedByThree) % colorMap1;
