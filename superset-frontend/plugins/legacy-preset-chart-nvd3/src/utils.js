...
// function to hide quarters when it is not selected #Periodicity custom code starts
export function generateRichLineTooltipContent(
  ...
  // Check the value of timegrain_store and set a flag to include fiscal info
  var timegrain_store = localStorage.getItem("timegrain_store");
  let includeFiscalInfo = false;

  if (timegrain_store !== null && timegrain_store !== undefined) {
    timegrain_store = JSON.parse(timegrain_store);
    if (timegrain_store === "P3M") {
      includeFiscalInfo = true;
    }
  }
  // Create a function to calculate the fiscal quarter label
  const calculateFiscalQuarterLabel = timestamp => {
    const date = new Date(timestamp);
    const fiscalYearStartMonth = 3; // Fiscal year starts in April (0-based index)
    const month = date.getMonth();
    const adjustedMonth = (month - fiscalYearStartMonth + 12) % 12; // Adjust for fiscal year starting in April
    const fiscalMonths = ["Q1", "Q2", "Q3", "Q4"];
    const fiscalQuarter = Math.floor(adjustedMonth / 3);
    const startMonthName = date.toLocaleString('default', { month: 'short' });
    const endMonthDate = new Date(date);
    endMonthDate.setMonth(date.getMonth() + 2); // Add two months for the end of the quarter
    const endMonthName = endMonthDate.toLocaleString('default', { month: 'short' });
    
    return `${fiscalMonths[fiscalQuarter]} ${startMonthName}-${endMonthName}`;
  };
  ...
  d.series.forEach(series => {
    ...
    if (includeFiscalInfo) {
      const fiscalQuarterLabel = calculateFiscalQuarterLabel(series.data.x);
      tooltip +=
        `<td><strong>${fiscalQuarterLabel}</strong><br>${key}</td>` +
...
// function to hide quarters when it is not selected #Periodicity custom code ends
