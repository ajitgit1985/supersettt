...
export default function DataSourcePanel({
  ...
  // const martixValues = columnSlice.filter(col => martixTypes.includes(col.type)).length;
  const martixValues = columnSlice.filter(col =>
    martixTypes.includes(col.type!),
  ).length;
  ...
  // Count columns with valid types
  const columnValues = columnSlice.filter(col => {
    const typeMatch = col.type?.match(/^(\w+)/); // Use optional chaining here
        ...
        <div className="field-selections">
          ...
          >
            {metrics?.length && (
              <Collapse.Panel
                header={
                  <SectionHeader>{t('Parameters Metrics')}</SectionHeader>
                }
                key="metrics" // Unique key for this panel
              >
            ...
            <Collapse.Panel
              header={<SectionHeader>{t('Dataset Metrics')}</SectionHeader>}
              key="column" // Unique key for this panel
            >
              <div className="field-length">
                {t(
                  `Showing %s of %s`,
                  martixValues, // Use the martixValues here
                  lists.columns.length,
                )}
              </div>
              {columnSlice.map(col => {
                if (col.type && martixTypes.includes(col.type)) {
                  return (
                    <LabelContainer
                ...
                return null; // Don't render columns with invalid types
              })}
            ...
            <Collapse.Panel
              header={<SectionHeader>{t('Dataset Dimensions')}</SectionHeader>}
              key="dimensions" // Unique key for this panel
            >
              <div className="field-length">
                {t(`Showing %s of %s`, columnValues, columnSlice.length)}
              </div>
              {columnSlice.map(col => {
                ...
                return null; // Don't render columns with invalid types or no type
              })}
              {lists.columns.length > DEFAULT_MAX_COLUMNS_LENGTH ? (
                <ButtonContainer>
                  <Button onClick={() => setShowAllColumns(!showAllColumns)}>
