...
// import { genderValues } from 'src/views/atoms';
import { columnTypes } from '../../../../../../superset-frontend/plugins/helpingMethoads/methods';
...
function DndColumnSelect(props: DndColumnSelectProps) {
  ...
  const [columnFilteredOptions, setColumnFilteredOptions] = useState<
    Array<any>
  >([]);

  // console.log('select column values >>>> ', options);

  // const genderValuesOnTooltip = useAtomValue(genderValues);
  // console.log('props values ', genderValuesOnTooltip);
  ...
  // console.log('select column values 55555555 ', optionSelector);
  // console.log('select column values 99999999 ', value);

  // useEffect(() => {
  // const a = options.filter(item => item.column_name === 'Gender');
  // const c = optionSelector.values.filter(item => item.column_name === 'Gender');

  // useEffect(()=>{})
  // if (genderValuesOnTooltip && a.length !== 0) {
  //   if (c.length === 0) {
  //     optionSelector.values.push(a[0]);
  //     // value.push('Gender');
  //     onChange(optionSelector.getValues());
  //   }
  // } else {
  //   if (c.length === 1) {
  //     optionSelector.values.splice(
  //       optionSelector.values.findIndex(a => a.column_name === 'Gender'),
  //       1,
  //     );
  //     // value.splice(
  //     //   value.findIndex(a => a === 'Gender'),
  //     //   1,
  //     // );
  //     onChange(optionSelector.getValues());
  //   }
  // }
  // }, [genderValuesOnToolt1ip, optionSelector.values, value]);
  // else if (!l) {
  //   optionSelector.values.splice(
  //     optionSelector.values.findIndex(a => a.column_name === c.column_name),
  //     1,
  //   );
  //   value.splice(
  //     value.findIndex(a => a === 'Gender'),
  ...
  // }
  // } else {
  //   // d = optionSelector.values.filter(item => item.column_name !== 'Gender');
  //   if (l) {
  //     optionSelector.values.splice(
  //       optionSelector.values.findIndex(a => a.column_name === c.column_name),
  //       1,
  ...
  //   }
  // }
  // }, [optionSelector.values, value]);
  // console.log('select column values ppppppppp', d);
  ...
  const onClickClose = useCallback(
    (index: number) => {
      console.log(
        'select column values delete values : ',
    ...
    () =>
      optionSelector.values.map((column, idx) => {
        ...
        return (
          ...
          >
            {/* {genderValuesOnTooltip &&
            column?.column_name === 'Gender' ? null : ( */}
            <OptionWrapper
              key={idx}
              index={idx}
  ...
  // filter the column values in popover box

  useEffect(() => {
    const columnValues = options.filter((col: any) =>
      columnTypes.includes(col.type),
    );
    setColumnFilteredOptions(columnValues);
  }, [options]);

  return (
    <div>
      {/* Dimansion / column div box */}
      <DndSelectLabel
        onDrop={onDrop}
        canDrop={canDrop}
        ...
        onClickGhostButton={openPopover} //  for DIMENSIONS
        {...props}
      />
      {/* Dimansion / column popover box */}
      <ColumnSelectPopoverTrigger
        columns={columnFilteredOptions}
