...
// previously defined function custom functionality added for the Periodicity.
const ActionButtons = ({
  width = OPEN_FILTER_BAR_WIDTH,
  onApply,
  onClearAll,
  ...
  // functionality for custom quarter tooltip, store filter type in session for tooltip (year, month, quirter)
  for (const key in dataMaskSelected) {
    if (dataMaskSelected.hasOwnProperty(key)) {
      if (key) {
        const dynamicKey = key; // This will be something like "NATIVE_FILTER-HcOeUwVSI"
        const timeGrainSqla =
          dataMaskSelected[key].extraFormData?.time_grain_sqla;
        localStorage.setItem('timegrain_store', JSON.stringify(timeGrainSqla));
      }
    }
  }
  // end of code of functionality for custom quarter tooltip
