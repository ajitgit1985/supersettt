...
function formatValue(value: number) {
  ...
  if (absoluteValue >= 1000) {
    // Convert to thousands (Th )
    return `₹ ${float2PointFormatter(value / 1000)}Th`;
  }
...
export default function createINRUnits(
  config: {
    description?: string;
    signed?: boolean;
    id?: string;
  ...
  return new NumberFormatter({
    ...
    id:
      id || signed ? NumberFormats.SMART_NUMBER_SIGNED : NumberFormats.INRunits,
